...
class MyProblem(Problem):
    ...
    def __init__(self, features, imp, sim):

        # Lower and upper bounds
        xl = anp.array([0]*features).astype(int)
        xu = anp.array([1]*features)#.astype(int)
        
        super().__init__(n_var=features, n_obj=2, n_constr=0, xl=xl, xu=xu)
    ...
    def function1(self, xx):
        ...
        for y in listOfOnes:
            value = 0
            for i in y:
                value = value - self.importance_scores[i]
            output.append(value)
...
def runNSGA2_unconstrained(dataset_name, num_features, pop_size, ranking, imp_metric, sim_metric, fold):#More parameters to be added     
    ...
    problem = MyProblem(num_features, score_dataframe[imp_metric], sim_scores_file) 
    ...
    np.savetxt(output_path+dataset_name+"/fs_unconstrained/"+ranking+"_"+dataset_name+"_"+fold+"_"+sim_metric+"_"+imp_metric+".txt", res.X.astype(int), delimiter="", fmt='%s')