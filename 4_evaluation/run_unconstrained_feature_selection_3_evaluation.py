...
similarity_metrics = ["kendall","spearman"]
importance_metrics = ["MAP@10","MRR@10","F1@10"]
pop_size = 100
        ...
        for simi in similarity_metrics:
            for imp in importance_metrics:
                with concurrent.futures.ThreadPoolExecutor() as executor:
                    fold_results = executor.map(ee.evaluation, itertools.repeat(rank), itertools.repeat(dataset_name), itertools.repeat(simi), itertools.repeat(imp), itertools.repeat("unconstrained"), fold_ints, parameters)
                    temp = str(list(fold_results))
                    f = open("../output/"+dataset_name+"/fs_unconstrained_evaluation/"+rank+"_"+dataset_name+"_"+simi+"_"+imp+"_evaluation.txt", "w")
                    f.write(temp)
                    f.close()